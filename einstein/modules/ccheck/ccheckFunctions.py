...
#   10/09/2009  HS  Small correction in "constrain" function: interpretation of
#                   NUMERIC_ERR as RELATIVE error instead of absolute error.
#
#------------------------------------------------------------------------------        
...
#------------------------------------------------------------------------------
    def __init__(self, name, parType=None, priority=3):
#------------------------------------------------------------------------------
        self.name = name
...
#------------------------------------------------------------------------------
    ...
    def update(self, new):
        """Set all numeric values to the ones given in new."""
        self.val = new.val
...
# is required in checkPipe for setting feed-up flow to EXACTLY 0
            ...
            self.val = 1.0
            self.sqerr = 0
        elif self.parType in ("T", "TC") and err == DEFAULT_SQERR:
            self.sqdev = 0.1
            self.calcErr()
        else:
            self.sqerr = err
...
#------------------------------------------------------------------------------
        ...
        if (self.valMin > self.valMax + max(abs(self.valMin), abs(self.valMax)) * NUMERIC_ERR):
...
#------------------------------------------------------------------------------
#   adds the current estimate to the stack
#------------------------------------------------------------------------------
        currentEstimate = CCPar(self.name + "[%2d]" % len(self.stack))
        if new is None:
            currentEstimate.update(self)
        else:
            currentEstimate.update(new)
        self.stack.append(currentEstimate)
...
#------------------------------------------------------------------------------
        ...
        self.push() # now everything is in the stack
        valList = []
        for entry in self.stack:
            valList.append(entry.val)
            
        bal = cycle.checkBalance(valList)
        
        if DEBUG in ["ALL", "CHECK"]:
            print "CCPar.check(before...)________________________________"
            for entry in self.stack:
                entry.show()
        ...
        n = len(self.stack)
        if CHECKMODE == "BEST":
            y = bestOfRow(self.stack, n)
        else:
            y = meanOfRow(self.stack, n)
...
#------------------------------------------------------------------------------
def calcProd_SYM(y, x1, x2):
#------------------------------------------------------------------------------
#   Default function for calculating the product of two values
#------------------------------------------------------------------------------
    ynew = calcProd(y.name, x1, x2)
    x1new = CCPar(x1.name)
    x1new.update(x1)
    x2new = CCPar(x2.name)
    x2new.update(x2)
    adjustProd(y, x1new, x2new)
    x1.push(x1new)
    x2.push(x2new)
    y.push(ynew)
...
def calcSum(yname, x1, x2, parType=None):
    ...
    @param yname: the name for the resulting CCPar object
    @param x1: the one summand
    @param x2: the other summand
    @param parType: the parType for the resulting CCPar object 
    """
...
def calcRowSum(name, row):
    """Calculate the sum of a list of CCPar objects."""
    y = CCPar(name)
    (nones, dummy) = countNones(row)
...
def calcFlow(Qdotname, cp, m, T1, T0, DT, DT1):
    """
    Calculate the power required to increase the temperature of a mass
    flow m at a starting temperature T0 up to the final temperature T1.
    :returns: the required power
    """
...
#------------------------------------------------------------------------------
def adjustcalcK_stack(y, a, x1_old):
#------------------------------------------------------------------------------
#   Default function for calculating and adjusting the product between a value and a constant
#------------------------------------------------------------------------------
    x1 = CCPar(x1_old.name)
    x1.update(x1_old)
...
#------------------------------------------------------------------------------
def adjustProd_stack(y, x1_old, x2_old):
    """
    Adjust two values of which the product is known.
    
    The new adjusted values are pushed on their respective stacks.
    If y, x1_old and x2_old are lists or tuples with common length 
    adjustProd_stack is applied for each triple of entries.
    
    :param y: known product
    :param x1_old: one factor
    :param x2_old: the other factor

    """
    # are the input parameters are list/tuples of compatible length? 
    if (all(isinstance(para, (type([]), type(()))) for para in (y, x1_old, x2_old))
         and len(y) == len(x1_old) == len(x2_old)):
        for i in xrange(len(y)):
            adjustProd_stack(y[i], x1_old[i], x2_old[i])
    else: # no they are not:
        x1 = CCPar(x1_old.name)
        x2 = CCPar(x2_old.name)
        x1.update(x1_old)
        x2.update(x2_old)
...
def adjustSumOfProds_stack(y, x1, x2):    
    """
    Adjust lists of corresponding factors, where the sum of the products is
    known.
    
    The Sum should be of the form:
    sum over n of (x1_n * x2_n)
    The new, adjusted values are pushed on their respective stacks.
    
    :param y: the known sum
    :param x1: the one list of factors
    :type x1: list of CCPar objects
    :param x2: the other list with the corresponding factors
    :type x2: list of CCPar objects
    """
    assert(len(x1) == len(x2))
    products = [calcProd("temp. Product %s" % (i,), x1[i], x2[i]) 
                for i in xrange(len(x1))]
    adjRowSum(y, products, len(x1))
    for i in xrange(len(products)):
        adjustProd_stack(products[i], x1[i], x2[i])

def adjustSumOfProdsC_stack(y, a, x1, x2):    
    """
    Adjust two lists of corresponding variable factors and keep a list
    of constant factors as is, where the sum of the products is known.
    
    The Sum should be of the form:
    sum over n of (a_n * x1_n * x2_n)
    The new, adjusted values are pushed on their respective stacks.
    
    :param y: the known sum
    :param a: the list of constant factors 
    :param x1: the one list of factors
    :type x1: list of CCPar objects
    :param x2: the other list with the corresponding factors
    :type x2: list of CCPar objects
    """
    assert(len(x1) == len(x2) == len(a))
    products = [calcProdC("temp. Product %s" % (i,), a[i], x1[i], x2[i]) 
                for i in xrange(len(x1))]
    adjRowSum(y, products, len(x1))
    for i in xrange(len(products)):
        adjustProdC_stack(products[i], a[i], x1[i], x2[i])
...
#------------------------------------------------------------------------------
def adjustProdS_stack(y, x1_old, x2_old):
#------------------------------------------------------------------------------
#   Default function for calculating the product of two values
#------------------------------------------------------------------------------
    x1 = CCPar(x1_old.name)
...
#------------------------------------------------------------------------------
def adjustProdC_stack(y, a, x1_old, x2_old):
    """
    Calculate and adjust the variable factors of a product among two values and
    a constant that has a known result.
    
    The new adjusted values are pushed on their respective stacks.
    If y, a, x1_old and x2_old are lists or tuples with common length 
    adjustProd_stack is applied for each quadruple of entries.
    
    :param y: the known product 
    :param a: the constant factor
    :param x1_old: the one variable factor
    :param x2_old: the other variable factor
    """
    # are the input parameters are list/tuples of compatible length? 
    if (all(isinstance(para, (type([]), type(()))) for para in (y, a, x1_old, x2_old))
         and len(y) == len(x1_old) == len(x2_old)):
        for i in xrange(len(y)):
            adjustProdC_stack(y[i], a[i], x1_old[i], x2_old[i])
    else: # no they are not:
        x1 = CCPar(x1_old.name)
        x2 = CCPar(x2_old.name)
        x1.update(x1_old)
...
#------------------------------------------------------------------------------
    ...
    if iszero(y):   #special case SUM = 0, valid if all numbers are positive
        ...
        if not (x1.val == None) and ((x1.sqdev < x2.sqdev) or x2.val == None):
            ...
            x2.calcDev() # TODO: Why is it calculated a second time? See before "if".
...
#------------------------------------------------------------------------------
def adjustSum_stack(y, x1_old, x2_old):
#------------------------------------------------------------------------------
#   Default function for calculating the product of two values
#------------------------------------------------------------------------------
    x1 = CCPar(x1_old.name)
...
#------------------------------------------------------------------------------
def adjustSumS_stack(y, x1_old, x2_old): #Adjusting simultaneously both values, not only the worst
#------------------------------------------------------------------------------
#   Default function for calculating and adjusting the components of a sum of 2 values
#------------------------------------------------------------------------------
    x1 = CCPar(x1_old.name)
...
#------------------------------------------------------------------------------
def adjustSum3_stack(y, x1_old, x2_old, x3_old): #Adjusting any value not only the worst
#------------------------------------------------------------------------------
#   Default function for calculating and adjusting the components of a sum of 3 values
#------------------------------------------------------------------------------
    x1 = CCPar(x1_old.name)
    x2 = CCPar(x2_old.name)
    x3 = CCPar(x3_old.name)
    x1.update(x1_old)
    x2.update(x2_old)
    x3.update(x3_old)
...
#------------------------------------------------------------------------------
def adjRowSum_stack(y, row_old, m):
    """
    Adjust given values and estimate the ones that are None such that
    their sum is as prescribed.
    
    The new values are pushed on their respective stacks, and the 
    absolute difference of the incoming summands and the given
    sum is returned.
    
    :param y: The known sum
    :param row_old: a list of CCPar summands
    :param m: the number of items in row_old that are to be adjusted.
      the first m items are adjusted.
    """
    assert(m == len(row_old))
    row = []
    for i in range(len(row_old)):
        row.append(CCPar(row_old[i].name))
        row[i].update(row_old[i])
...
#------------------------------------------------------------------------------
def adjustDiff_stack(y, x1_old, x2_old, GTZero=False):
    """
    Adjust two values of which the difference is known.
    
    The new adjusted values are pushed on their respective stacks.
    If y, x1_old and x2_old are lists or tuples with common length 
    adjustDiff_stack is applied for each triple of entries.
    
    :param y: known difference
    :param x1_old: the minuend
    :param x2_old: the subtrahend

    """
    # are the input parameters list/tuples of compatible length? 
    if (all(isinstance(para, (type([]), type(()))) for para in (y, x1_old, x2_old))
         and len(y) == len(x1_old) == len(x2_old)):
        for i in xrange(len(y)):
            adjustDiff_stack(y[i], x1_old[i], x2_old[i])
    else: # no they are not:
        x1 = CCPar(x1_old.name)
        x2 = CCPar(x2_old.name)
        x1.update(x1_old)
...
def adjustFlow_stack(Qdot, cp, m_old, T1_old, T0_old, DT_old, DT1_old):
    """
    Adjust values of which the flow is known.
    
    The new adjusted values are pushed on their respective stacks.
    If all parameters are lists or tuples adjustFlow_stack is applied
    for each 7-tuple of entries.
    """
    # are the input parameters list/tuples of compatible length? 
    if all((isinstance(para, (type([]), type(()))) 
           for para in (Qdot, cp, m_old, T1_old, T0_old, DT_old, DT1_old))):
        for i in xrange(len(Qdot)):
            adjustFlow_stack(Qdot[i], cp[i], m_old[i], T1_old[i], T0_old[i],
                             DT_old[i], DT1_old[i])
    else: # no they are not:
        m = CCPar(m_old.name)
        T1 = CCPar(T1_old.name)
        T0 = CCPar(T0_old.name)
        DT = CCPar(DT_old.name)
        DT1 = CCPar(DT1_old.name) 
        m.update(m_old)
        T1.update(T1_old)
        T0.update(T0_old)
        DT.update(DT_old)
        ...
        adjustProdC_stack(Qdot, cp, m, DT)
...
#------------------------------------------------------------------------------
#   Default function for adjusting the enthalpy h
#------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------
def adjustH_stack(y, cL, cV, h, T0, x1_old, x2_old):
#------------------------------------------------------------------------------
#   Default function for adjusting the enthalpy h
#------------------------------------------------------------------------------
    x1 = CCPar(x1_old.name)
...
#------------------------------------------------------------------------------
    ...
    # FIXME: check calc/adjConstraintLT/GT comments, directions and undefined variables
...
#------------------------------------------------------------------------------
def adjConstraintGT_stack(y, xlim_old):
#------------------------------------------------------------------------------
#   Changes y so that y <= xlim
#------------------------------------------------------------------------------
    xlim = CCPar(xlim_old.name)
    xlim.update(xlim)
    xlim_old.push(calcConstraintLT(xlim, y))
