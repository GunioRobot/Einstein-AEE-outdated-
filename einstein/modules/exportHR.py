...
#   20/04/2010: DB Export detailed schedules  
#   
#------------------------------------------------------------------------------     
...
class SQLViews(object):
    ...
    SQL_ColdGeneration = """SELECT q.`Equipment`, q.`HCGPnom`, q.`PartLoad`, q.`HPerDayEq`, q.`NDaysEq`, d.`THighP`, d.`TCond`, 
                            ...
                            ORDER BY q.`EqNo`;"""
    SQL_ProcessData    = """SELECT q.`Process`, q.`ProcType`, q.`HPerDayProc`, q.`VolProcMed`, q.`StartUpDuration`, q.`InFlowDuration`, q.`OutFlowDuration`, q.`ScheduleTolerance`, q.`HolidayScale`
    ...
    SQL_ProcessData2   = """SELECT q.`NBatch`, q.`HBatch`, q.`NDaysProc`, q.`QOpProc`, q.`HPerDayProc`, q.`QEvapProc`, q.`StartUpDuration`, q.`InFlowDuration`, q.`OutFlowDuration`, q.`ScheduleTolerance`, q.`HolidayScale`  
                            FROM qprocessdata q
...
class XMLDocHRModuleBase:
    ...
    def handleSpecialCases(self,value,tag):
        # Convert integer to float for HolidayScale
        if (tag=="HolidayScale"):
            if value == None:
                return "holiday_scale_none"
            else:
...
class XMLDocHRModuleAll(XMLDocHRModuleBase):
        ...
        for pid in pids:
            ...
            for process in processes:
                ...
                for value in process: 
                    ...
                    convertedvalue = self.handleSpecialCases(value,(self.procedata[self.INNERTAGS])[count])                    
                    if (convertedvalue!=None):
                        innerentryvalue = self.document.createTextNode(convertedvalue) ##XX
                        innerentry.appendChild(innerentryvalue)
                    entry.appendChild(innerentry)
    ...
    def HandleSchedules(self,parent):
        ...
        # write out industry holiday periods
        holidays = self.document.createElement("HolidayPeriods")
        for holidayInterval in schedules.exportHolidayIntervalsFromDB(self.qid):
            holidayElement = self.document.createElement("Holidays")
            start = self.document.createElement("Start")
            stop  = self.document.createElement("Stop")
            start.appendChild(self.document.createTextNode(str(holidayInterval["start"])))
            stop.appendChild(self.document.createTextNode( str(holidayInterval["stop"])))
            holidayElement.appendChild(start)           
            holidayElement.appendChild(stop)
            holidays.appendChild(holidayElement)
        entry.appendChild(holidays)
        for scheduleList in [Status.schedules.procOpSchedules,
                             # FIXME: delete or handle backward compatible
...
# 20100702 DB Disabled since new HR module is not available yet       
#        try:
#            detailedSchedule = schedule.detailedSchedule
#        except AttributeError: # no detailed schedule available
#            pass
#        else:
#            yearlyProfile = self.document.createElement("YearlyProfile")
#            for (intervalStart, intervalStop, intervalScale) in detailedSchedule.getYearlyProfileIntervals():
#                start = self.document.createElement("Start")
#                stop  = self.document.createElement("Stop")
#                scale = self.document.createElement("Scale")
#                start.appendChild(self.document.createTextNode("%2f" % intervalStart))
#                stop.appendChild(self.document.createTextNode( "%2f" % (intervalStop + 1))) # stop hour is inclusive
#                scale.appendChild(self.document.createTextNode("%2f" % intervalScale))
#                yearlyProfile.appendChild(start)
#                yearlyProfile.appendChild(stop)
#                yearlyProfile.appendChild(scale)
#            process.appendChild(name)
#            process.appendChild(type)
#            process.appendChild(yearlyProfile)
...
#            return
    ...
    procdatatags    = ["Process","ProcType","HPerDayProc","VolProcMed", "StartUpDuration", "InFlowDuration", "OutFlowDuration", "ScheduleTolerance", "HolidayScale"]
