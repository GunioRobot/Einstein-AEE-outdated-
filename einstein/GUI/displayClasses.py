...
class FloatEntry(wx.Panel):
    ...
    def OnUnits(self, event):
        ...
        self.defaultDisplayUnit = event.GetString().encode(ENCODING)
        # recalculate to display units
        self.SetValue(self.lastInternalValue)
        # allow descendant handlers
        event.Skip()
    ...
    def GetValue(self):
        dValue = self.entry.GetValue().encode(ENCODING)
        if dValue is None:
            return None
...
#                print "DisplayClasses (ChoiceEntry - SetValue): value = %r"%thing
                ...
                except TypeError: # thing is unicode already
                        pass
