...
from einstein.GUI.numCtrl import convertDoubleToString
...
class PanelEM2(wx.Panel):
    def __init__(self, parent):
        ...
        self.keys = ['EM2_Table','EM2_Plot']
        self._showHeating = True 
        self.mod = ModuleEM2(self.keys, self._showHeating)
    ...
    def display(self):
        ...
        if self._showHeating:
            self.box1.SetLabel(_U('Monthly useful supply heat'))
            self.box2.SetLabel(_U('Distribution of useful supply heat per months'))
            plottitle = _U('Monthly process heat supply')
        else:
            self.box1.SetLabel(_U('Monthly useful cooling supply'))
            self.box2.SetLabel(_U('Distribution of useful cooling supply per months'))
            plottitle = _U('Monthly process cooling supply')
        labels_column = 0
        ...
        paramList={'labels'      : labels_column,                 # labels column
                   ...
                   'key'         : self.keys[1],                       # key for Interface
        ...
        Mp.MatPanel(self.panelGraphMPHS,wx.Panel,drawStackedBarPlot,paramList)
        
        # grid setup
        try:
            self.grid1.Destroy()
        except AttributeError:
            # There was no self.grid1 to destroy in the first place
            pass
        # set grid properties
